...
export class VentilatorController {
    ...
    private registerRoutes(): void {
        this.app.get("/api/ventilator/state", (req: Request, res: Response) => {
            let ventilatorState = this.getState();
        ...
        this.app.put("/api/ventilator/speed/:speed", (req: Request, res: Response) => {
            const speed = parseInt(req.params.speed, 10);

            if (isNaN(speed) || speed < 1 || speed > 3) {
                return res.status(400).json({ error: "Speed must be a number between 1 and 3." });
            }
