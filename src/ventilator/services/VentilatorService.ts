...
export class VentilatorService {
    ...
    public async setSpeed(desiredSpeed: number): Promise<void> {
        ...
        this._ventilatorState.speed = newSpeed <= VentilatorService.MAX_SPEED ? newSpeed : newSpeed % VentilatorService.MAX_SPEED;
        console.log(
    ...
    public async rotate(): Promise<void> {
        if (this._ventilatorState.isOn && await this.ventilatorTerminal.rotate()) {
