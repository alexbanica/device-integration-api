...
class VentilatorConfiguration {
    ...
    private validateConfiguration(): void {
        if (!this._ventilatorWorkingDirectory) {
            throw new VentilatorConfigurationError('VENTILATOR_WORKING_DIRECTORY is required');
        }
        if (!this._ventilatorStartCommand) {
            throw new VentilatorConfigurationError('VENTILATOR_START_COMMAND is required');
        }
        if (!this._ventilatorStopCommand) {
            throw new VentilatorConfigurationError('VENTILATOR_STOP_COMMAND is required');
        }
        if (!this._ventilatorRotateCommand) {
            throw new VentilatorConfigurationError('VENTILATOR_ROTATE_COMMAND is required');
        }
        if (!this._ventilatorSpeedCommand) {
            throw new VentilatorConfigurationError('VENTILATOR_SPEED_COMMAND is required');
        }
    }
